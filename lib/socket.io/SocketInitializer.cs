...
namespace socket.io {
    ...
    public class SocketInitializer : MonoBehaviour {
        ...
        public string WebSocketUrl {
            get {
				var builder = new StringBuilder(BaseUrl.Replace("http://", "ws://").Replace("https://", "wss://"));
                builder.Append("/socket.io/");
        ...
        /// WebSocket URL
        /// </summary>
        public string NamespaceQuery {
            get {
                var builder = new StringBuilder();

                for (int i = 0; i < _queries.Count; ++i) {
                    var elem = _queries.ElementAt(i);
                    builder.Append(i == 0 ? "?" : "&");
                    builder.Append(elem.Key + "=" + elem.Value);
                }
        ...
        IEnumerator InitCore(IObserver<Socket> observer, CancellationToken cancelToken)
        {
            ...
            if (Socket.HasNamespace)
                webSocketTrigger.WebSocket.Send(new Packet(EnginePacketTypes.MESSAGE, SocketPacketTypes.CONNECT, Socket.Namespace + NamespaceQuery, string.Empty).Encode());
