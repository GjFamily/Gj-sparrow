...
void directShare(const char* text, const char* imagePath, const char* title,const char* targeturl, const char* videourl,int platform, ShareDelegate callback){
...
//    NSLog(@"targetUrl:%@", nstargeturl);
    ...
    if (nstargeturl==nil||nstargeturl.length==0) {
        ...
        //创建视频内容对象
        UMShareVideoObject *shareObject = [UMShareVideoObject shareObjectWithTitle:nstitle descr:[NSString stringWithUTF8String:text] thumImage:image];
        //设置视频网页播放地址
